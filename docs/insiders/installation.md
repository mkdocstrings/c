...
*mkdocstrings-c Insiders* is a compatible drop-in replacement for *mkdocstrings-c*,
...
*mkdocstrings-c Insiders* can be installed with `pip` [using SSH][using ssh]:
...
Self-hosting the Insiders package makes it possible to depend on *mkdocstrings-c* normally,
...
with [Twine]:
  ...
  [Github Releases]: https://github.com/pawamoy-insiders/mkdocstrings-c/releases
...
>
> Now when running `pip install mkdocstrings-c`,
...
Of course, you can use *mkdocstrings-c Insiders* directly from `git`:
...
When upgrading Insiders, you should always check the version of *mkdocstrings-c*
